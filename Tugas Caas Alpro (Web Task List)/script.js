...
function addMember() {
    ...
    if (!memberName) {
        showSuccessPopup('Harap isikan nama anggota!');
        return;
    }

    if (members.includes(memberName)) {
        showSuccessPopup(`Anggota ${memberName} sudah ada!`); 
    } else {
...
function addNewTask() {
    ...
    if (!newTask) {
        showSuccessPopup('Harap isikan nama tugas!');
        return;
    }

    if (tasks.includes(newTask)) {
        showSuccessPopup(`Tugas ${newTask} sudah ada!`); 
    } else {
        tasks.push(newTask);
...
function assignTask() {
    ...
    if (selectedMember === '' || selectedTask === '' || dueDateInput === '') {
        showSuccessPopup('Harap lengkapi semua kolom sebelum menugaskan tugas!');
    } 
    if (!selectedMember) {
        showSuccessPopup('Harap pilih anggota sebelum menugaskan tugas!');
        return;
    }
    if (!selectedTask) {
        showSuccessPopup('Harap pilih tugas sebelum menugaskan!');
        return;
    }
    if (!dueDateInput) {
        showSuccessPopup('Harap pilih tenggat waktu sebelum menugaskan!');
        return;
    }
        ...
        if (taskAssignedToMember) {
            showSuccessPopup(`Tugas ${selectedTask} sudah ditugaskan ke ${selectedMember}!`);
        } else {
...
function resetMembers() {
    ...
    if (members.length === 2 && members.includes('Ellen') && members.includes('Lycaon')) {
        showSuccessPopup('Tidak ada anggota baru yang telah ditambahkan!');
    } else {
...
function resetTasks() {
    ...
    if (tasks.length === 4 && tasks.includes('Alpro') && tasks.includes('OOP') && tasks.includes('PPB') && tasks.includes('PBO')) {
        showSuccessPopup('Tidak ada tugas baru yang telah ditambahkan!');
    } else {
...
function resetAssignments() {
    ...
    if (assignments.length === 0) {
        showSuccessPopup('Tidak ada penugasan yang telah ditambahkan!');
    } else {
